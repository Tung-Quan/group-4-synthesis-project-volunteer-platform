...
@router.post("/apply")
def apply_event(request: application_models.ApplyEvent, current_user: dict = Depends(get_current_user)):
...
def attendance(event_id: str, request: application_models.UpdateAttendance, current_user: dict = Depends(get_current_user)):
...
def review_app(event_id: str, request: application_models.ReviewApplication, current_user: dict = Depends(get_current_user)):
...
def cancel_application_route(event_id: str, request: application_models.CancelApplication, current_user: dict = Depends(get_current_user)):
